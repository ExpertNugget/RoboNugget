...
class admin(commands.Cog):
    ...
    @commands.has_permissions(admin=True)### this isn't working for some reason, will figure out tmr -Nugget
    async def set_staff(self, ctx, role: discord.Option(discord.Role)):
        await ctx.defer()
        with sqlite3.connect(database) as conn:
            cur = conn.cursor()
            cur.execute("INSERT OR REPLACE INTO config (guild_id, admin_role_id) VALUES (?, ?)", (ctx.guild.id, role.id,)) 
